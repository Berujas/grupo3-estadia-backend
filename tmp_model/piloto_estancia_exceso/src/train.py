...
def main(args):
    ...
    X_train, X_test, y_train, y_test, num_cols, cat_cols = make_dataset(config)
    ...
    lr = LogisticRegression(max_iter=300, class_weight="balanced", solver="saga", C=1.0)
    lr_pipe = Pipeline([("prep", preproc), ("clf", lr)])
    ...
    # --- Modelo potente: HistGradientBoosting + calibración ---
    hgb = HistGradientBoostingClassifier(random_state=42, class_weight="balanced")
    hgb_pipe = Pipeline([("prep", preproc), ("clf", hgb)])
    # Calibración con CV dentro de entrenamiento
    hgb_cal = CalibratedClassifierCV(hgb_pipe, method="isotonic", cv=config["training"]["calibration_cv"])
    ...
    # Gráficos y reportes
    plot_and_save_roc(y_test, hgb_proba, os.path.join(paths["reports_dir"], "roc.png"))
    plot_and_save_pr(y_test, hgb_proba, os.path.join(paths["reports_dir"], "pr.png"))
    plot_and_save_calibration(y_test, hgb_proba, os.path.join(paths["reports_dir"], "calibration.png"))
    ...
    # Preview con probabilidades (primeras 200 filas del test)
    import pandas as pd
