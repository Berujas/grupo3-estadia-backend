...
Versión corregida - maneja mejor las probabilidades bajas
"""
import pandas as pd
import sys
import os
...
def predict_nuevos_pacientes_fixed():
    """
    Predice exceso de estadía para pacientes nuevos
    Versión corregida que maneja mejor las probabilidades bajas
    """
    
    print("🏥 SISTEMA DE PREDICCIÓN PARA PACIENTES NUEVOS (VERSIÓN CORREGIDA)")
    print("=" * 70)
    ...
    output_file = os.path.join(carpeta_nuevos, "predicciones.csv")
    ...
    try:
        ...
        # Crear resultados con categorización mejorada
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Sistema de predicción para pacientes nuevos (versión corregida)")
    parser.add_argument("--predecir", action="store_true", help="Ejecutar predicción")
    
    args = parser.parse_args()
    
    if args.predecir:
        result = predict_nuevos_pacientes_fixed()
        ...
        print("   2. Ejecuta: python predict_nuevos_pacientes_fixed.py --predecir")
        print("   3. Revisa los resultados en 'nuevos_pacientes/predicciones.csv'")
        print()
        print("🔧 CORRECCIONES:")
        print("   - Maneja mejor las probabilidades bajas (0.0)")
        print("   - Categorización mejorada para evitar valores 'nan'")
        print("   - Resultados más interpretables")
